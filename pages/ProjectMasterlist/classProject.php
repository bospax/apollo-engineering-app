...
class ProjectList
{
	...
	public function UpdateProjectStatus($OrganizationNumber, $P_status){
	 ...
	 public function UpdateProjectAmount($PCode, $BudgetedAmt){
			 
		$stmts = $this->conn->prepare("UPDATE apollo_projectlist SET budgeted_amount =:Budgeted_Amt WHERE project_code=:PCode");
		
				$stmts->bindparam(":PCode",$PCode);
