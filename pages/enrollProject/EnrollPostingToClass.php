...
if ($row ==0) {
    if($Percent == "100"){
        if($EnrollNewProject->EnrollNew($ProjectCode, $ProjectName, $CapexNumber, $OrgNumber, $Contractor, $AssignEo, $project_status, $NtpDate, $ContractAmount, $DownPayment, $ProjectRetention,$Proponent, $Payee, $StartDate, $EndDate, $PlannerName)){
            if($EnrollNewProject->AddDownPayment($BillingDate, $CapexNumber, $ProjectName, $Contractor, $ContractAmount, $fullPayment, $BillingProgress, $Progress_amount, $DownPayment, $billing_status)){
                if($EnrollNewProject->AddDpToFirstApproverTable($BillingDate, $CapNum, $Ca, $Contractors, $fullPayment, $d_Progress, $BillableAmount,$BillingStatus, $ApprovalDate, $ApproverName, $Remarks)){
