...
$username = "webdomainadmin_apollogantt"; // replace with actual username
$password = "eiqJdETZ.BE8"; // replace with actual password
$dbname = "webdomai_engineering"; // replace with actual database
...
// $connect = mysqli_connect("148.72.214.239", "webdomai_Apollo", "apolloproject", "webdomai_engineering");
...
if(mysqli_num_rows($result) > 0)
{
 ...
 {  
    ...
    array_push($taskPlan, 
        ...
        'tooltext'  =>$a.' '."Day/s Delayed",
            ...
            $getStart = "SELECT MIN(planned_start)as startDate FROM apollo_project_assigned_scopes Where capex_number = '$CapexNum'";
            ...
            if(mysqli_num_rows($resultDate) > 0)
            {
             ...
             if(mysqli_num_rows($resultEnd) > 0)
             {
                ...
                $getSlack = "SELECT sum(TIMESTAMPDIFF(day,planned_end,actual_end)) as 'Slack'
 ...
 $result = array("scope"=>$SomeScope,"PlanStartActualEnd"=>$somePlanStartActualEnd,"somePlanEndActualEnd"=>$somePlanEndActualEnd,
 "taskPlan"=>$taskPlan,"StartDate"=>$startDate,"EndDate"=>$EndDate,"arrayRangeYear"=>$arrayYearRange,"arrayWeekly"=>$arrayWeekly,
 "arraySlacks"=>$TotalSlackDelay, "ProjectName"=>$ProjectName );
 $forProjectScope = json_encode($result);
 echo $forProjectScope;
//  var_dump($forProjectScope);
}else{
