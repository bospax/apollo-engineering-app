<?php
    ...
    require_once("ProjectMonitoring/class.php");
                ...
                <div class="row align-items-center">
                    <div class="col-sm-6">
                        <div class="breadcrumbs-area clearfix">
                            <h4 class="page-title pull-left" id="ContainerTitle">Pending Works</h4>
                    ...
                    <div class="col-12 mt-5" id="AssignedProjectList">
                        <div class="card">
                            <div class="card-body">                              
                                ...
                                <h4 class="header-title">Pending To Start Works</h4>  <HR>
                                <div align="right">
                                       <div>
                                ...
                                <div>
                                    ...
                                    <?php   
                                         if(isset($_SESSION['position'])){
                                            ...
                                            if($Position == 'Eo'){
                                                $view = $ProjectMonitoring->runQuery("SELECT capex_number, project_name FROM apollo_enrolledproject WHERE engineer='$fullname'");
                                                $view->execute();
                                            }
                                            else{
                                                $view = $ProjectMonitoring->runQuery("SELECT capex_number, project_name FROM apollo_enrolledproject");
                                                $view->execute();
                                            }
                                        }
                                    ...
                                    <!--<input type="text" class="form-control col-md-3 mb-3" id="Pname" value="" placeholder="Contractor" readonly>-->
...
function SearchCapexNum(val){
        ...
        $.ajax({
        type: "POST",
        url: "ProjectMonitoring/SearchContractor.php",
        data: {capex:val},
        dataType: "json",
        success: function(response){   
            $('#Contractor').val(response.Contractor); 
            $('#Pname').val(response.ProjectName); 
            // console.log(response);
        }
...
function SetStartDate(btnval){
    var r = confirm("You are Requesting to Start This Work?");
    if (r == true) {
    
    // alert ("You pressed OK!");
           $.ajax({
            url:"AssignedWorks/SetActualStart.php",
            method:"POST",
            data:{workId : btnval},
            success:function(message)
            {
               ...
               alert(message);
                ...
                // console.log(response);
                setTimeout(function(){
                    $( "#data-tables" ).load( "PendingToStartWorks.php #data-tables" );
                    reloadData();
                    }, 250);
            }
...
function RejectWorks(WorkId){
    var r = confirm("You are Requesting to Reject This Work?");
    if (r == true) {
    
    // alert ("You pressed OK!");
           $.ajax({
            url:"ProjectMonitoring/PostingRejectedWorks.php",
            method:"POST",
            data:{WorkId : WorkId},
            success:function(rejected)
            {
                $('#AlertSucess').html(rejected);            
